...
def init():
    ...
    sr_headers = load_headers_data('../docs/sr_headers')
...
for index in range(len(sr_headers)):
    print 'index : ' + str(index) 
    for uid in temp_uids[i:]:
        print uid
        result = get_user_sr(uid, sr_headers[index])
        if result == 2:
            break
        elif result != 1:
