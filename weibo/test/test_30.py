...
class Gas():
    ...
    def fun(self,x): #定义函数
        ...
        tags_w = weight[2]
        statuses_w = weight[3]
        user_sim = baseinfo_w*self.baseinfo_sim + sr_w*self.sr_sim + \
                   tags_w*self.tags_sim + statuses_w*self.statuses_sim
        eq = self.get_eq(user_sim)
        return eq
...
if __name__ == '__main__':
    generation = 50 # 遗传代数数
    mainGas = Gas(30, 30 , 0 , 1, 4)#引入Gas类，传入参数：种群大小，编码长度，变量范围, 变量个 
    pop = mainGas.initialpop()  #种群初始化
    pop_best = [] #每代最优个体
    for i in xrange(generation):#在遗传代数内进行迭代 
